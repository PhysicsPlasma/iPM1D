    ...
    contains
         subroutine DiagParticleTestParticle(PB,FG,Mode)
            ...
            Select Case (Mode)
                       ...
                       TempPB%Npar=1
                       TempPB%Timer=0
                       TempPB%PhaseSpace(1)%X=1.d0
                       TempPB%PhaseSpace(1)%Vx=0.d0
                       TempPB%PhaseSpace(1)%Vy=0.d0
                       TempPB%PhaseSpace(1)%Vz=0.d0
                       TempPB%PhaseSpace(1)%Ax=0.d0
                       TempPB%PhaseSpace(1)%Ay=0.d0
                       TempPB%PhaseSpace(1)%Az=0.d0
                       TestPB=TempPB
                       TestPB%Timer=1
                       ...
                       Case(0)
                           If (TempPB%Timer<=NParMax) Then
                               If ((TempPB%PhaseSpace(1)%X>TempPB%XMin).and.(TempPB%PhaseSpace(1)%X<TempPB%XMax)) Then
                               Call ParticleMove(TempPB,FG)
                               TempPO=TempPB%PhaseSpace(1)
                 ...
                 Case(-1)
                        ...
                        Case(0)
                            ...
                            do i=TempPB%NPar,1,-1
                                 If (TempPB%PhaseSpace(i)%X<=TempPB%XMin) then
                                     TempPB%PhaseSpace(i)%Ay=dble(ParticleTimer)
                                     Call AddParticle(TempPB%PhaseSpace(i),PE%PBLower)
                                 else If(TempPB%PhaseSpace(i)%X>=TempPB%XMax) then
                                     TempPB%PhaseSpace(i)%Ay=dble(ParticleTimer)
                                     Call AddParticle(TempPB%PhaseSpace(i),PE%PBUpper)
                                 end if
                            end do
                            PE%Timer=PE%Timer+1
                       Case(1)
            ...
            Integer(4) :: Shift,i
                     Select Case (Mode)
                        ...
                        Case(0)
                            ...
                            Do i=1,2
                                Call GridUpdate(GD,PE%Nflux,PE%Flux(1:PE%NFlux,i),Shift)
                                Call GridUpdate(GD,PE%Nflux,PE%EnergyFlux(1:PE%NFlux,i),Shift)
                            END DO
                            GD%Timer=GD%Timer+1
                         Case(1) 
                                 Call GridRescale(GD)
...
!    !            ParticleTemp%Ay=0.0
!    !            ParticleTemp%Az=0.0
!    !        ParticleTemp%Vx=0.d0!InputParticle%PhaseSpace(NRandom)%Vx
!    !        ParticleTemp%Vy=0.d0!InputParticle%PhaseSpace(NRandom)%Vy
!    !        ParticleTemp%Vz=0.d0!InputParticle%PhaseSpace(NRandom)%Vz
!    !        !Call Maxwellian(TraEGas,ParticleTemp)    
!    !        trajectory%PhaseSpace(Npar1)=ParticleTemp
!    !        
!    !        ParticleTempBundle%PhaseSpace(1)=ParticleTemp
!    !        Return 
!    !Entry DiagTrajectory(InputPICParticlePara,InputNx)
!    !   If (trajectory%PhaseSpace(Npar1)%X>XMin.and.trajectory%PhaseSpace(Npar1)%X<XMax) then
...
!    !       Timer=Timer+1
!    !        NPar1=NPar1+1
!    !        trajectory%PhaseSpace(Npar1)=ParticleTempBundle%PhaseSpace(1)
!    !        Return 
...
!    !   end subroutine DiagTrajectoryINit
     ...
     Subroutine GridUpdate(GD,Nx,A1D,Shift)
         Implicit none
         Class(*),intent(inout)  :: GD
  ...
  subroutine WeightingParticleMomentum(PB,PMO)
                ...
                do i=1,PB%Npar
                   ...
                   PMO%JxOne(N)=PMO%JxOne(N)+S1*PB%PhaseSpace(i)%Vx
                   PMO%JxOne(N+1)=PMO%JxOne(N+1)+S2*PB%PhaseSpace(i)%Vx

                   Call CalEnergy (PB%Mass,PB%PhaseSpace(i),Energy)
                ...
                TFactor=PB%Weight*PB%VFactor*PB%VFactor/JtoEV
    ...
    subroutine WeightingParticleEDF(PB,PEDF)
        ...
        subroutine WeightingParticleCollisionRate(PB,PCR,MCCB)
                ...
                do j=1,PB%Npar
                   Call CalEnergy (PB%Mass,PB%PhaseSpace(j),Energy)
                   Energy=Energy*PB%VFactor*PB%VFactor/JtoeV
                   TempProbility=0.d0 
        ...
        subroutine WeightingParticleFlux(PE)
                ...
                do i=1,PE%PBLower%Npar
                   Call CalEnergy (PE%PBLower%Mass,PE%PBLower%PhaseSpace(i),Energy)
                   Energy=Energy*PE%PBLower%VFactor*PE%PBLower%VFactor/JtoeV
                   !Write(*,*) Energy,"Lo"
                   Timer=Int(PE%PBLower%PhaseSpace(i)%Az)/NAva+1
                ...
                do i=1,PE%PBUpper%Npar
                   Call CalEnergy (PE%PBUpper%Mass,PE%PBUpper%PhaseSpace(i),Energy)
                   Energy=Energy*PE%PBUpper%VFactor*PE%PBUpper%VFactor/JtoeV
                   !Write(*,*) Energy,"Up"
                   Timer=Int(PE%PBUpper%PhaseSpace(i)%Az)/NAva+1
                   If(Timer>=1.and.Timer<=PE%NFLux) Then
